...
bool GdmaDma::init() {
  ...
  // Frame-boundary semaphore. The last descriptor of each chain has
    // suc_eof = 1, so this is given once per full chain traversal.
    ...
    gdma_cbs.on_trans_eof = GdmaDma::frame_eof_isr;

    esp_err_t cb_err = gdma_register_tx_event_callbacks(dma_chan_, &gdma_cbs, this);

    if (cb_err != ESP_OK) {
      ESP_LOGE(TAG, "Failed to register GDMA EOF callback: %s", esp_err_to_name(cb_err));
      vSemaphoreDelete(frame_sem_);
      frame_sem_ = nullptr;
      return false;
    }
...
void GdmaDma::shutdown() {
  ...
  if (dma_chan_) {
    ...
    // Only after the channel is gone -- no ISR can reference it now.
    if (frame_sem_) {
      vSemaphoreDelete(frame_sem_);
      frame_sem_ = nullptr;
    }
...
bool IRAM_ATTR GdmaDma::frame_eof_isr(gdma_channel_handle_t chan,
                                      gdma_event_data_t *event_data,
                                      void *user_data) {
  (void) chan;
  (void) event_data;

  GdmaDma *self = static_cast<GdmaDma *>(user_data);
  BaseType_t hp_task_woken = pdFALSE;

  if (self && self->frame_sem_) {
    xSemaphoreGiveFromISR(self->frame_sem_, &hp_task_woken);
  }
...
void GdmaDma::flip_buffer() {
  ...
  const TickType_t kEofTimeout = pdMS_TO_TICKS(100);

  // ---------------------------------------------------------------------
  // Step 1: sync to a frame boundary BEFORE splicing.
  //
  // GDMA latches a descriptor's `next` pointer when it loads the descriptor,
  // not when the transfer completes. Splicing while DMA is already on (or
  // prefetching) the last descriptor is therefore too late for this pass --
  // DMA loops back into the old chain, but EOF still fires. Waiting on that
  // EOF would free a buffer that is about to be scanned again.
  //
  // Landing the splice right after a frame boundary puts it a full chain
  // away from the end, which closes that window.
  // ---------------------------------------------------------------------
  if (frame_sem_) {
    xSemaphoreTake(frame_sem_, 0);  // discard any stale give

    if (xSemaphoreTake(frame_sem_, kEofTimeout) != pdTRUE) {
      ESP_LOGW(TAG, "flip_buffer: no EOF before splice (DMA stalled?)");
    }
  }

  // ---------------------------------------------------------------------
  // Step 2: splice the chains (three stores -- microseconds at most).
  //   Before: A_last -> A_first (circular)
  //   After:  A_last -> B_first, B_last -> B_first
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  // Step 3: block until DMA reaches the splice and jumps to the new chain.
  // Only then is the old front buffer -- now the CPU drawing target --
  // genuinely off screen and safe to clear().
  // ---------------------------------------------------------------------
  if (frame_sem_) {
    if (xSemaphoreTake(frame_sem_, kEofTimeout) != pdTRUE) {
      ESP_LOGW(TAG, "flip_buffer: no EOF after splice (DMA stalled?)");
    }
  }
}
